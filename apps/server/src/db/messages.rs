...
/// One row of the "Threads you're in" aggregate.  Each row corresponds to a
/// distinct *parent* message whose thread the requesting user has either
/// authored a reply in, or been mentioned in via a reply.
///
/// `unread_reply_count` is currently a coarse approximation derived from the
/// conversation-level `read_receipts.last_read_at` — when there's no per-thread
/// read state yet, any reply newer than the user's last_read_at counts as
/// unread.  TODO: introduce a `thread_reads (thread_id, user_id, last_read_at)`
/// table for per-thread precision (#449).
#[derive(Debug, sqlx::FromRow, serde::Serialize)]
pub struct ParticipatedThreadRow {
    pub parent_message_id: Uuid,
    pub conversation_id: Uuid,
    pub channel_id: Option<Uuid>,
    /// Truncated ciphertext / plaintext of the parent message.  Clients
    /// decrypt when the conversation is E2E; otherwise render verbatim.
    /// Bounded server-side (`LEFT(.., 240)`) so the response payload is
    /// predictable even for very long parents.
    pub parent_preview: Option<String>,
    /// Username of the parent message author.  Useful for the
    /// "Re: <user>'s message" affordance on the threads screen.
...
/// Fetch the list of threads the user has participated in (authored a reply
/// OR been mentioned in within a reply), ordered by most-recent reply first.
///
/// Pagination uses an optional `before` cursor on `last_reply_at`; callers
/// page by passing the timestamp of the oldest row in the previous batch.
pub async fn get_participated_threads(
    pool: &PgPool,
    user_id: Uuid,
    before: Option<DateTime<Utc>>,
    limit: i64,
) -> Result<Vec<ParticipatedThreadRow>, sqlx::Error> {
    // The CTE collects parent message ids the user has touched via reply
    // (either as the reply's author, or by being mentioned in a reply).
    // We then aggregate reply stats once per parent and join author info.
    //
    // Membership is enforced implicitly: a parent message only appears if
    // a reply rooted in it exists in the same conversation the user is a
    // member of.  The final WHERE clause double-checks active membership
    // so soft-removed members don't keep seeing threads after a kick.
    sqlx::query_as::<_, ParticipatedThreadRow>(
        "WITH participated AS ( \
             SELECT DISTINCT r.reply_to_id AS parent_id \
             FROM messages r \
             WHERE r.reply_to_id IS NOT NULL \
               ...
               AND ( \
                   r.sender_id = $1 \
                   OR EXISTS ( \
                       SELECT 1 FROM mentions mm \
                       WHERE mm.message_id = r.id \
                         AND mm.mentioned_user_id = $1 \
                   ) \
               ) \
         ), \
         stats AS ( \
             SELECT r.reply_to_id AS parent_id, \
                    COUNT(*) AS reply_count, \
                    MAX(r.created_at) AS last_reply_at \
             FROM messages r \
             JOIN participated p ON p.parent_id = r.reply_to_id \
             WHERE r.deleted_at IS NULL \
             GROUP BY r.reply_to_id \
         ) \
         SELECT p.id AS parent_message_id, \
                p.conversation_id, \
                p.channel_id, \
                LEFT(p.content, 240) AS parent_preview, \
                pu.username AS parent_sender_username, \
                s.reply_count, \
                ( \
                    SELECT COUNT(*) FROM messages r2 \
                    WHERE r2.reply_to_id = p.id \
                      AND r2.deleted_at IS NULL \
                      AND r2.sender_id <> $1 \
                      AND r2.created_at > COALESCE(rr.last_read_at, 'epoch'::timestamptz) \
                ) AS unread_reply_count, \
                s.last_reply_at, \
                ( \
                    SELECT lu.username FROM messages lr \
                    JOIN users lu ON lu.id = lr.sender_id \
                    WHERE lr.reply_to_id = p.id \
                      AND lr.deleted_at IS NULL \
                    ORDER BY lr.created_at DESC \
                    LIMIT 1 \
                ) AS last_reply_sender_username \
         FROM stats s \
         JOIN messages p ON p.id = s.parent_id AND p.deleted_at IS NULL \
         JOIN conversation_members cm \
         ...
         LEFT JOIN read_receipts rr \
                ON rr.conversation_id = p.conversation_id AND rr.user_id = $1 \
         WHERE ($2::timestamptz IS NULL OR s.last_reply_at < $2) \
