...
// ---------------------------------------------------------------------------
// Minimal binary fixtures for avatar upload tests
// ---------------------------------------------------------------------------

/// 1x1 pixel PNG (valid magic bytes).
...
/// Plain ELF header bytes -- rejected by magic-byte check.
...
// ---------------------------------------------------------------------------
// Avatar upload: PUT /api/users/me/avatar
// ---------------------------------------------------------------------------

#[tokio::test]
async fn avatar_upload_valid_png_returns_200() {
    let base = common::spawn_server().await;
    let client = Client::new();
    let (token, _) = setup_user(&client, &base, "avpng").await;
    ...
    assert_eq!(resp.status().as_u16(), 200, "valid PNG should be accepted");
    let body: Value = resp.json().await.unwrap();
    assert!(
        body["avatar_url"].as_str().is_some(),
        "response must include avatar_url"
    );
}
...
async fn avatar_upload_non_image_returns_4xx_with_code() {
    let base = common::spawn_server().await;
    let client = Client::new();
    let (token, _) = setup_user(&client, &base, "avelf").await;

    // Send ELF bytes but claim image/png -- magic-byte check must reject it.
    ...
    assert!(
        ...
        "non-image upload must be rejected with 4xx, got {status}"
    );
    let body: Value = resp.json().await.unwrap();
    let code = body["code"].as_str().unwrap_or("");
    assert!(
        code == "unsupported-media-type" || code == "bad-request",
        "expected unsupported-media-type or bad-request code, got \'{code}\'"
    );
}
